...
3. Select ```Cluster``` on the left-hand menu and click on "Create cluster" button. You will be directed to the ```Configure cluster``` page:
    ...
    - Fill the ```Name``` and ```Kubernetes version``` fields. (Ex: MyCluster, 1.28)
...
12. Atfer clean-up `worker nodes` and `cluster`, delete the `LoadBalancer` manually. ec2